	...
	for(var i in questions){
		...
		for(var j = 0; j<questions[i].options.length; j++){
			...
			if(i !=='3'){
				...
				musicbox.onclick = function (){play(j)};
				div3.appendChild(musicbox);
			}
...
function play(n){
	let audio = obj.getElementsByTagName('audio')[n];
	if (audio.readyState === 4){
		if (audio.paused) {
			audio.play();
			$(this).style.background = "url('../img/pause.png') no-repeat center";
		} else {
			audio.pause();
			$(this).style.background = "url('../img/play.png') no-repeat center";
		}
}
	return true;
}
